...
def menuAddCharacter():
	...
	while (flag == True):
		...
		char.mapKey = input("Enter map key: ")
		char.petFood = input("Enter pet food key: ")
		char.changeChannel = input("Enter change channel key: ")
...
def writeToFile():
	...
	#writing reset skills section
	file.write("[resets]\n")
	for i in range(char.resetSkillsNum):
		file.write("skill" + str(i + 1) + " = " + str(char.resetSkills[i].name) + ", " + str(char.resetSkills[i].key) + "\n")
	file.write("\n")
...
def readFromFile(charName):
	...
	char.changeChannel = configure.get("general", "changeChannel")
...
def addResetSkill():
	...
	#add new skill to list and write to file
	char.resetSkills.append(Skill(name, key))
	char.resetSkillsNum = char.resetSkillsNum + 1
	writeToFile()
